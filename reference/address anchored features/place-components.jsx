...
function SectionCard({
  ...
  caption,
  chip,        // { tone, text, icon }
  statusDot,   // color string -> small dot before inline value
  sparkline,   // bool
  ...
  // ── INLINE: compact single-line reading (denser rhythm) ──
  if (inline && (state === 'loaded' || state === 'stale')) {
    return (
      <div className="pl-card" style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 11 }}>
        <IconTile name={icon} tone={tone} size={32} />
        <div style={{ fontSize: 15, fontWeight: 600, color: INK, letterSpacing: '-0.01em', flexShrink: 0 }}>{title}</div>
        <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 7 }}>
          {chip ? (
            <Chip tone={chip.tone} icon={chip.icon}>{chip.text}</Chip>
          ) : action ? (
            <span style={{ color: SKY, fontWeight: 600, fontSize: 14, whiteSpace: 'nowrap' }}>{action.label}</span>
          ) : (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 15, fontWeight: 500, color: INK, textAlign: 'right', lineHeight: '19px' }}>
              {statusDot && <span style={{ width: 8, height: 8, borderRadius: '50%', background: statusDot, flexShrink: 0 }} />}
              {value}
            </span>
          )}
        </div>
  ...
  return (
    <div className="pl-card pl-hero" style={{ padding: '16px 16px 14px' }}>
      ...
      <div style={{ display: 'flex', gap: 13, alignItems: 'flex-start', marginBottom: 14 }}>
        <div style={{ width: 42, height: 42, borderRadius: 12, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: alert ? '#FFFBEB' : HOME_GREEN_BG, border: `1px solid ${alert ? '#fde68a' : '#bbf7d0'}` }}>
          <Icon name={alert ? 'wind' : 'shield-check'} size={22} color={alert ? '#b45309' : HOME_GREEN} strokeWidth={2} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <button className={`pl-nudge ${alert ? 'alert' : ''}`}>
        <Icon name={alert ? 'clock' : 'lightbulb'} size={17} color={alert ? '#b45309' : HOME_GREEN} strokeWidth={2} />
        <span style={{ flex: 1, textAlign: 'left', fontSize: 13.5, color: INK2, lineHeight: '19px' }}>
          {alert
            ? 'Limit time outdoors this afternoon. It should clear by evening.'
            : 'A heat-pump rebate may apply to your home. Worth a look.'}
        </span>
        <Icon name="chevron-right" size={17} color="#b8bdc6" strokeWidth={2.25} />
      </button>
    </div>
  );
}

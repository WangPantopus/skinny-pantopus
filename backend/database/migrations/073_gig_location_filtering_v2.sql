-- ============================================================
-- Migration 073: Gig Location Filtering V2
-- ============================================================
-- Adds upgraded spatial query RPCs for gigs:
--   - find_gigs_nearby_v2 (location filtering, remote gigs, search, sort, pagination)
--   - find_gigs_in_bounds_v2 (remote gigs support, category filtering, enriched columns)
--
-- Mirrors the patterns in find_listings_nearby_v2 / find_listings_in_bounds_v2.
-- Old functions are kept for backward compatibility.
-- ============================================================


-- ─── 1. RPC: find_gigs_nearby_v2 ────────────────────────────
-- Replaces find_gigs_nearby with:
--   • ST_DWithin for index usage (instead of ST_Distance <= radius)
--   • Remote gig support (exact_location IS NULL)
--   • Category / price / search filtering
--   • Multi-column sort (newest, price_asc, price_desc, distance)
--   • LIMIT / OFFSET pagination
--   • Enriched return columns (city, state, tags, items, etc.)
--   • Creator info from User join
...
-- ─── 2. RPC: find_gigs_in_bounds_v2 ─────────────────────────
-- Extends find_gigs_in_bounds with:
--   • Remote gig support (approx_location IS NULL items in side list)
--   • Category filtering
--   • Enriched return columns matching find_gigs_nearby_v2

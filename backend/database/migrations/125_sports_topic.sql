...
-- Mirror of supabase/migrations/20260423000001_sports_topic.sql
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Post_origin_check') THEN
    ALTER TABLE "public"."Post"
      ADD CONSTRAINT "Post_origin_check"
      CHECK ("origin" IN ('user', 'curator', 'system'));
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Post_sports_scope_check') THEN
    ALTER TABLE "public"."Post"
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Post_topic_check') THEN
    ALTER TABLE "public"."Post"
...
CREATE INDEX IF NOT EXISTS idx_post_topic_created
  ON "public"."Post" ("topic", "created_at" DESC) WHERE "topic" IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_post_topic_scope_created
  ON "public"."Post" ("topic", "sports_scope", "created_at" DESC) WHERE "topic" = 'sports';
CREATE INDEX IF NOT EXISTS idx_post_metadata_gin
  ON "public"."Post" USING gin ("post_metadata" jsonb_path_ops);
CREATE INDEX IF NOT EXISTS idx_post_origin
  ON "public"."Post" ("origin") WHERE "origin" <> 'user';
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'seeder_content_queue_scope_check') THEN
    ALTER TABLE "public"."seeder_content_queue"

...
`20260922021100_gig_tip_original.sql` for original Payment reservation, frozen
...
The reservation migration is `20260922021100_gig_tip_original.sql`. It reuses Payment.id for both identities and financial columns for amounts/customer/provider IDs; only absent original terms and control state use metadata.gig_tip_original_v1. Unverified legacy rows remain unmarked. The forward function/index update `20260921021200_gig_tip_legacy_recovery.sql` registers only fresh-proven existing rows under an unchanged Payment snapshot. It adds no table or column. Native omitted historical dates normalize to null; new-charge admission still requires current nonnull terms. Cold historical discovery is implemented through the existing detail/picker preview. The forward `20260921021300_gig_tip_notification_delivery.sql` commits the existing Notification and Payment metadata delivery marker with a new capture; it adds no table or column. The existing scheduled payment relay leases the same notice, checks its frozen payment/content and capture-time/current preferences, and records accepted, suppressed or retryable transport outcomes without stale metadata writes. Uncertain transport may retry the same Notification ID (at least once); deletion, prior delivery and historical capture are not backfilled or re-alerted. Full installed/provider acceptance remains required before activation.

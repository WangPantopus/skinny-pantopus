...
const ref = { home_id: 'ddf10001-0000-4000-8000-000000000100', claim_id: 'ddf10001-0000-4000-8000-000000000200',
...
test('exact private key and immutable provider upload', async () => {
  ...
  expect(bucket.upload).toHaveBeenCalledWith(`claim-evidence/${ref.home_id}/${ref.claim_id}/${ref.upload_id}/${ref.sha256}`, file.buffer,
...
test('wrong existing bytes never count as upload success', async () => {
  ...
  await expect(storage.upload(ref, file.buffer)).rejects.toMatchObject({ code: 'HOME_CLAIM_EVIDENCE_UPLOAD_UNAVAILABLE' });
});
test('download verifies length and hash', async () => {
  ...
  await expect(storage.download(ref)).rejects.toMatchObject({ code: 'HOME_CLAIM_EVIDENCE_INTEGRITY' });
});
...
test.each(['home_id', 'claim_id', 'upload_id', 'sha256'])('rejects traversal in %s', field => expect(() => storage.key({ ...ref, [field]: '../other' })).toThrow());

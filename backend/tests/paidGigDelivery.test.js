...
const relay = require('../jobs/deliverGigAcceptance');
const note = { id: 'note', user_id: 'worker', type: 'bid_accepted', title: 'Accepted',
  body: 'Synthetic acceptance', link: '/gigs/gig', metadata: { gig_id: 'gig' } };
beforeEach(() => {
...
describe('stored notification delivery', () => {
  test('delivers the exact stored notification and never inserts another row', async () => {
    ...
    expect(push.sendToUserWithReceipt).toHaveBeenCalledWith('worker', expect.objectContaining({ data: {
      notificationId: 'note', type: 'bid_accepted', link: '/gigs/gig', gig_id: 'gig',
    } }));
...
describe('durable acceptance relay', () => {
  ...
  test('unknown or partial provider outcome stays retryable under the same event identity', async () => {
    ...
    await relay(); expect(rpc).toHaveBeenCalledWith('finish_gig_acceptance_delivery', {
  ...
  test('push opt-out settles as suppressed, preventing replay after preference restoration', async () => {
    ...
    await relay(); expect(rpc).toHaveBeenCalledWith('finish_gig_acceptance_delivery', expect.objectContaining({ p_outcome: 'suppressed' }));
  });

...
function key({ home_id: homeId, task_id: taskId, upload_id: uploadId, sha256 }) {
  if (![homeId, taskId, uploadId].every(id => typeof id === 'string' && UUID.test(id)) || !/^[0-9a-f]{64}$/.test(sha256 || '')) {
    throw failure('HOME_TASK_MEDIA_INVALID', 'Invalid attachment reference.', 400);
  }
  return `task-media/${homeId.toLowerCase()}/${taskId.toLowerCase()}/${uploadId.toLowerCase()}/${sha256}`;
}
function inspect(file) {
  const bytes = file?.buffer;
  if (!Buffer.isBuffer(bytes) || !bytes.length || bytes.length > MAX_BYTES) throw failure('HOME_TASK_MEDIA_SIZE', 'Choose a nonempty file of 25 MB or less.', 413);
  if (!MIME_TYPES.has(file.mimetype)) throw failure('HOME_TASK_MEDIA_TYPE', 'Choose a PDF, text file, or supported image.', 415);
  ...
  if (!matches) throw failure('HOME_TASK_MEDIA_TYPE', 'The file content does not match its type.', 415);
  const name = typeof file.originalname === 'string' ? file.originalname.normalize('NFC').replace(/[\x00-\x1f\x7f/\\]/g, '_').trim() : '';
  if (!name || name.length > 255) throw failure('HOME_TASK_MEDIA_INVALID', 'Choose a file with a name of 255 characters or less.', 400);
...
async function privateBucket(expected) {
  ...
  if (!/^[a-z0-9][a-z0-9-]{2,62}$/.test(name) || (expected !== undefined && expected !== name)) {
    throw failure('HOME_TASK_MEDIA_STORAGE_UNAVAILABLE', 'Private attachment storage is unavailable. Please retry.');
  }
  let result;
  try { result = await db.storage.getBucket(name); } catch (_) { throw failure('HOME_TASK_MEDIA_STORAGE_UNAVAILABLE', 'Private attachment storage is unavailable. Please retry.'); }
  if (result?.error || result?.data?.public !== false) throw failure('HOME_TASK_MEDIA_STORAGE_UNAVAILABLE', 'Private attachment storage is unavailable. Please retry.');
...
async function readBytes(bucket, ref) {
  ...
  if (result?.error || !result?.data || result.data.size !== ref.size || result.data.size > MAX_BYTES) {
    throw failure('HOME_TASK_MEDIA_DOWNLOAD_UNAVAILABLE', 'Could not verify this attachment. Please retry.');
  }
  ...
  if (bytes.length !== ref.size || crypto.createHash('sha256').update(bytes).digest('hex') !== ref.sha256) {
    throw failure('HOME_TASK_MEDIA_INTEGRITY', 'Could not verify this attachment. Please retry.');
  }
  return bytes;
}
async function upload(ref, bytes) {
  if (!Buffer.isBuffer(bytes) || bytes.length !== ref.size || bytes.length > MAX_BYTES
    ...
    throw failure('HOME_TASK_MEDIA_INVALID', 'Attachment bytes do not match the saved upload.', 400);
  }
  ...
  try {
    ...
    try { await readBytes(bucket, ref); } catch (_) { throw failure('HOME_TASK_MEDIA_UPLOAD_UNAVAILABLE', 'The task is saved, but this file was not confirmed. Retry the attachment.'); }
  }
}
...
async function remove(ref) {
  ...
  if (!result || result.error) throw failure('HOME_TASK_MEDIA_DELETE_UNAVAILABLE', 'The attachment is hidden, but storage cleanup is pending. Retry removal.');
}

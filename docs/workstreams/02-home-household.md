## September 22 native Emergency Info edit — focused persistence repair verified end to end

The retained Android owner flow reproduced a concrete persistence gap after the accepted PR191 dismissal repair: Home dashboard → Emergency info → existing row → Edit opened the existing form, but `submitEdit` only committed a local draft. There was no Android PUT binding and no backend update route, so the change could not survive a server reload or another client. Source comparison found the existing Home Emergency POST/DELETE route, DTO shape, form, detail route and shared web client; an in-place extension was sufficient and no new screen, service, table, migration or test file was justified.

PR192 (`codex/native-emergency-edit-20260922`, commits `5f9e6cf7f`, `96fb8ce3f`, and `46c27c85d`) adds the row-scoped `PUT /api/homes/:id/emergencies/:emergencyId` beside the existing route, reuses the existing Android DTO/API/repository, submits edits through the existing form, retains the draft on a failed request, and reloads the existing detail list on route resume. The shared web SDK now exposes the same established PUT contract. Existing layout, styling and navigation remain unchanged.

The focused populated-row review found a concrete preservation failure in the first implementation: an owned synthetic row with `location=Cabinet 2` and details `{detail: Existing description, phone: +15551234567, custom_key: keep-this}` was edited through the retained Android UI by changing only its title. The real PUT returned 200 but sent `location:null` and only the visible `detail` key, and SQL lost the untouched location, phone and custom key. Commit `46c27c85d` repairs this in place by carrying the existing location and raw details through `EmergencyFormDraft`, merging only dirty visible keys, preserving legacy projections, and refusing to dismiss on a malformed successful readback. The synthetic populated row and UI-created helper row were deleted by exact id; matching HomeEmergency and HomeAuditLog counts are zero. The repaired client still needs a rebuilt retained-emulator journey because the single heavy Android slot is reserved by Stream3.

The rebuilt app was installed on retained `emulator-5556` and exercised against the live LAN backend. A real owner create produced one row, then Detail → Edit changed **Edit target** to **Edited target**; the PUT returned HTTP 200, SQL `HomeEmergency.label` and `updated_at` changed, and the detail screen showed **Edited target** after its resume-triggered GET. A temporary exact `HomePermissionOverride(home.edit=false)` then produced HTTP 403 for **Denied target**; the form stayed open with the draft visible and SQL remained **Edited target**. Removing only that override and pressing Save again returned HTTP 200 and the detail showed **Denied target**. No audit row was generated by this update contract.

The synthetic row and temporary permission override were deleted by exact id after verification. Final retained counts for the fixture home are `HomeEmergency=0`, `HomePermissionOverride=0`, and `HomeAuditLog=0`; loopback and LAN health are HTTP 200/database connected. Android `assembleDebug` succeeded and installed APK SHA256 is `0bf946398303059d917f20a0fa4fc7f2adc5050f02f720528ae7f2144d656cd7`; incremental `compileDebugKotlin` also passed after the import-only cleanup. `node --check backend/routes/home.js` and `git diff --check` pass. The package-local TypeScript check remains blocked by existing repository ES-lib/process/module-resolution errors in unrelated files; no error referenced this new SDK function.

Durable sanitized evidence is `/Users/yingpengwang/skinny-pantopus/.pantopus-recovery/audits/20260922-stream2-native-emergency-edit-r1/`, MANIFEST SHA256 `ea4043da4ac0f16424beb82e6e21c3f6c7bf05c0222811321f6827c9be9667ee`; the pending populated-row receipts remain private until rebuilt-client verification. The first PR192 CI run 35756738929 correctly caught one KtLint line-wrap violation in the new repository method; follow-up commit `96fb8ce3f` fixes only that style issue. Exact-head CI run 35760093498 for the preservation repair is in progress. The physical iPhone remains unavailable to CoreDevice (4016/unavailable), so no physical iPhone install or launch is claimed; the existing signed LAN-configured artifact and backend remain available for a connected device.

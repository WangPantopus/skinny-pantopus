...
| U02 | Workflow-level visual checks. **Sep22 Android payment screens** (payment card, refund sheet, tip sheet) at font scale 2.0 and in dark mode: all reflow and stay usable. A dark-mode **tip-sheet contrast defect** (title nearly invisible) is repaired in PR198 (`6f3436bf3`; light mode pixel-identical, dark legible). Audit `20260922-stream1-u02-android-payment-a11y-r1`, MANIFEST `26ca2ea8…b33b`. | Remaining reachable-screen large text/zoom/keyboard/screen-reader/contrast/dark-mode matrix. Layout proposals needing design approval: the "WINNER" badge squeezes at 2.0, and task-progress labels break mid-word. Other GigDetail sheets may share the dark-mode pattern (not exercised). |

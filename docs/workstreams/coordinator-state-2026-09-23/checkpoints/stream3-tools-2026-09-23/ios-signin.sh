...
/private/tmp/pantopus-tools/heavy-slot.sh acquire "stream3: iOS app reinstall for sign-in ($NAME)" | tail -1
xcrun simctl terminate $U app.pantopus.ios 2>/dev/null
xcrun simctl install $U $R/ios-apps/harness/Pantopus.app || { /private/tmp/pantopus-tools/heavy-slot.sh release; exit 1; }

...
(async () => {
  ...
  await page.getByPlaceholder('Subject (optional)').fill(`Stream2 RD sender-gate web compose ${label}`);
  ...
  if (sent.id) {
    fs.appendFileSync(__dirname + '/work/t7-made-mail-ids.txt', sent.id + '\n');

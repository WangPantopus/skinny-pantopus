// Harness-only: add one disposable synthetic user with the password from a private file; keeps extra-accounts.env format.
...
(async () => {
  ...
  const pw = fs.readFileSync(pwFile, 'utf8').trim(); const email = `s2-r06-${key.toLowerCase()}@example.com`;

...
set -u
LOG=/private/tmp/pantopus-stream2-r06-runtime/work/t31-ios-build2.log
: > $LOG
/private/tmp/pantopus-tools/heavy-slot.sh acquire "stream2: iOS sim build (native Earn honest, incremental) against 18142" >> $LOG 2>&1
...
xcodebuild -scheme Pantopus -configuration Debug -destination "platform=iOS Simulator,id=6F914A30-8585-4B05-9E05-94441675F10A" \
  ...
  build > /private/tmp/pantopus-stream2-r06-runtime/work/t31-ios-xcodebuild2.log 2>&1
echo "xcodebuild exit $? $(date -u +%FT%TZ)" >> $LOG
tail -3 /private/tmp/pantopus-stream2-r06-runtime/work/t31-ios-xcodebuild2.log >> $LOG

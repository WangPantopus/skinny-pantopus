// Web mailbox Tasks: "New from mail" -> draft -> "Post as Gig instead" -> (before) modal Post Task, or (after)
// the real task form, posted from the member's Home (that one geocoder answer is supplied in the browser).
// Usage: node web-mailbox-task-post-gig.cjs <who> <before|after> <label> <shot>
...
(async () => {
  ...
  page.on('response', async (r) => {
    ...
    const id = j && j.gig && j.gig.id; if (id) fs.appendFileSync(__dirname + '/work/t27-made-gig-ids.txt', id + '\n');
  ...
  await page.getByRole('button', { name: /New from mail/ }).first().click();
  await page.waitForTimeout(1000);
  const title = `Stream2 RD ${label}: clean the gutters`;
  await page.getByPlaceholder('What needs to be done?').fill(title);
  await page.getByPlaceholder('Add details...').fill('Synthetic Stream2 check: the front gutters are full of leaves.');
  await page.getByRole('button', { name: /Post as Gig instead/ }).click();
  await page.waitForTimeout(2500);
  const lines = async () => (await page.innerText('body')).split('\n').map((l) => l.trim()).filter(Boolean);
  console.log(`# web ${who}: mailbox Tasks -> New from mail -> Post as Gig instead [${label}]`);
  if (mode === 'before') {
    ...
    const t = await page.getByPlaceholder('e.g., Need help moving a couch').inputValue().catch(() => '(no title field)');
    const d = await page.getByPlaceholder(/Describe what you need/).inputValue().catch(() => '');
    console.log(`-> ${url.split('?')[0]} (prefill: ${url.includes('prefill=')}); title=${JSON.stringify(t)} description=${JSON.stringify(d)}`);
    ...
    const shown = (await lines()).find((l) => l.includes(title));

#!/bin/zsh
# Usage: build-ios.sh <label>   (builds the current worktree state into the Shared UX derived data)
LABEL="$1"
...
/private/tmp/pantopus-tools/heavy-slot.sh acquire "shared-ux: iOS build ($LABEL)"
...
xcodebuild -project Pantopus.xcodeproj -scheme Pantopus -configuration Debug \
  ...
  build > /private/tmp/pantopus-shared-ux-runtime/xcodebuild-$LABEL.log 2>&1
RC=$?
/private/tmp/pantopus-tools/heavy-slot.sh release
echo "build end $(date -u +%FT%TZ) rc=$RC"
tail -3 /private/tmp/pantopus-shared-ux-runtime/xcodebuild-$LABEL.log

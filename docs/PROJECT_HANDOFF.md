...
**September 24 Ballot build track (separate from stream integration):** the founder asked for a plan, design and build, with every screen matching the [Ballot canvas](https://claude.ai/artifact/KCuXBiAYYaX13gpoqUCdmq). The [implementation plan](ballot-implementation-plan-2026-09-24.md) corrects the guide's contracts against the code on master `6688fd84` and sets the canvas-matching rules. Branch `claude/blissful-dijkstra-n8r31h` holds the P0 slices, all behind the `ballot_p0` flag (off): backend (`ee386d83`, `2db2d532`), web (`73323bfa`, `a48bdd00`), iOS (`635e3fe3`, `7eb6fac8`) and Android (`e12a73d7`, `bdb43226`). The canvas (version 10) carries the proposed P0 boards for approval, including the animated "P0: the peel, told" and the after-the-election proposal (plan §11 item 7: the card through certification with official results links, then the governments view all year on the Civic page; not built). Verified here: backend Jest 6319 passed, privacy and migration gates, web Jest 1911 passed and the type gate, web screenshots against the rendered boards (cards 0.00–0.01% of pixels differ; governments finished frame 0.09%), and a web end-to-end run on a local stack with the real backend and app ([plan §10.1](ballot-implementation-plan-2026-09-24.md#101-web-end-to-end-run-september-24)). That run found and fixed the governments sheet rendering under the app shell. A later pass fixed the existing Civic page, which shows the election Ballot provides: web showed the day before and iOS a blank date tile (`4daeea8f`), and the past election read "0 days away" the week after it (`b6630803`). The warning-well calendar icons now stay 18px (`c5d69bcf`). iOS passed format, strict lint and token checks but was not compiled. Android passes the project's ktlint and detekt (run through a stand-alone harness) and the hex check, but was not compiled or unit-tested: the proxy refuses dl.google.com, which serves Google Maven. Open as [WangPantopus/skinny-pantopus#429](https://github.com/WangPantopus/skinny-pantopus/pull/429). Next: founder approval of plan §11, CI on that PR (`ios-ci` and `android-ci` compile the native code), then release checks 1–2 and device passes. Limits: the container cannot reach sos.wa.gov, vote.gov, clark.wa.gov, the Census geocoder or dl.google.com, and has no Xcode. No stream ownership, acceptance count or merge state changes.

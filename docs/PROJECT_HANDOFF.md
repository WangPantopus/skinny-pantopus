...
**September 24 Ballot build track (separate from stream integration):** the founder asked for a plan, design and build, with every screen matching the [Ballot canvas](https://claude.ai/artifact/KCuXBiAYYaX13gpoqUCdmq). The [implementation plan](ballot-implementation-plan-2026-09-24.md) corrects the guide's contracts against the code on master `6688fd84` and sets the canvas-matching rules. The work happens on branch `claude/blissful-dijkstra-n8r31h`. Everything stays behind the `ballot_p0` flag, which is off. Next: proposed P0 state boards on the canvas, then the backend, web and native P0 slices. Limits: the build container cannot reach sos.wa.gov, vote.gov, clark.wa.gov or the Census geocoder, and has no Xcode or Android SDK. Release checks 1–2 and device passes remain open (plan §10–11). No stream ownership, acceptance count or merge state changes.

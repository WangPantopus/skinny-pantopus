...
**September 24 Ballot build track (separate from stream integration):** the founder asked for a plan, design and build, with every screen matching the [Ballot canvas](https://claude.ai/artifact/KCuXBiAYYaX13gpoqUCdmq). The [implementation plan](ballot-implementation-plan-2026-09-24.md) corrects the guide's contracts against the code on master `6688fd84` and sets the canvas-matching rules. Branch `claude/blissful-dijkstra-n8r31h` holds the P0 slices, all behind the `ballot_p0` flag (off): backend (`ee386d83`, `2db2d532`), web (`73323bfa`, `a48bdd00`), iOS (`635e3fe3`, `7eb6fac8`) and Android (`e12a73d7`, `bdb43226`). The canvas (version 7) carries the proposed P0 boards for approval, including the animated "P0: the peel, told". Verified here: backend Jest 6319 passed, privacy and migration gates, web Jest 1911 passed and the type gate, and web screenshots against the rendered boards (cards 0.00–0.01% of pixels differ; governments finished frame 0.09%). iOS passed format, strict lint and token checks but was not compiled. Android passes the project's ktlint and detekt (run through a stand-alone harness) and the hex check, but was not compiled or unit-tested: the proxy refuses dl.google.com, which serves Google Maven. Next: founder approval of plan §11, then a PR so `ios-ci` and `android-ci` compile the native code, then release checks 1–2 and device passes. Limits: the container cannot reach sos.wa.gov, vote.gov, clark.wa.gov, the Census geocoder or dl.google.com, and has no Xcode. No stream ownership, acceptance count or merge state changes.

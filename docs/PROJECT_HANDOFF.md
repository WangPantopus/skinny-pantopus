...
**September 24 web geo `center` readers (`claude/happy-noether-4skgca`):** `GET /api/geo/autocomplete` returns each suggestion's `center` as GeoJSON `[lng, lat]`; the backend and native decoders are unchanged. Three untyped web readers treated it as `{ lat, lng }`. Each was repaired in place to read `[lng, lat]`, with no design, type, backend or new-file change: Home Settings `geocodeAddress()` (behind both Auto-detect and Re-detect), and the `/resolve`-fallback coordinates of `feed/PostLocationPicker` and `gig-v2-create/QuickModifiersV2`. Reproduced on master `6688fd84` and re-verified in Chromium through Next dev, with the real `routes/geo.js`, Mapbox provider (only its outbound HTTP faked) and `updateHomeSchema` validator. Before: Auto-detect showed "Coordinates set" over empty fields, and Save sent no `location` (not NaN), so the home reverted to "No coordinates". When `/resolve` returned 500 or null coordinates, the feed composer posted a Portland-labelled place at the viewer's Seattle viewing area, and Post to Pulse sent no coordinates. After: the fields fill 45.5152 / -122.6784 and persist; both composers send the picked place's coordinates; `QuickModifiersV2` emits finite values. Web Jest 122 suites / 1893 tests, `type-check:gate` 0 errors, lint 0 errors with no new warnings. Limits: no Supabase, live Mapbox, deployed or native run. `POST /api/posts` was recorded, not executed; its 400 at `routes/posts.js:1160` for a coordinate-less `nearby` post is source-derived. `QuickModifiersV2` has no mounted screen on master, so it was exercised through a temporary, uncommitted route. The harness and screenshots stayed in the session scratchpad. `claude/brave-planck-fkmzrs` (typed `GeoSuggestion.center`, `suggestionCoordinates()`, and the `/start`, `LocationPicker` and `magic-task-v2` readers) is on neither origin nor master, so those four readers still misread `center` here. Next: publish and land that branch. Note for it: `tests/unlistedWeb.test.tsx` still feeds `StartFunnel` a `{ lat, lng }` suggestion `center`.

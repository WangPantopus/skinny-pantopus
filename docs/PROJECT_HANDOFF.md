...
**September 25 Ballot end-to-end pass (PR #429 branch):** web, the iOS app on an iPhone 17 simulator and the Android app on an API 35 emulator were driven against a local stack with the real backend ([plan §10.2](ballot-implementation-plan-2026-09-24.md#102-web-ios-and-android-end-to-end-run-september-25)). Found and fixed on the branch: a 404 Clark County drop-box link; native "Open your ballot" landing on the Place hub with no card; an 8-second whole-page wait when the Census geocoder is slow or down (now a 3.5-second budget, a working 7-day stale window and one geocoder call per home); the web crashing its whole Place page on a malformed Ballot field; the web governments view clipped on short laptop windows and without a focus trap; the Android view under the status bar; wrong web timeline text in 7 p.m. states; a contradictory "not configured" row under the `/start` teaser; and "0 days away" on the Civic tile. Local gates run: web eslint and type gate, backend privacy gates, ktlint, detekt, SwiftLint strict, SwiftFormat, both native builds. Unit suites were not run here (founder direction). The founder approved plan §11 items 1–6 the same day and chose "City of Denver". A new government-counting rule follows the Census of Governments (plan §11 item 8): consolidated city-counties count once, and Honolulu reads "at least 3" (United States, the state, the City and County of Honolulu) without the state-run Department of Education. It was checked with live Census data, on the web and on Android. Next: green PR CI, merge with the flag off, release check 2 (real addresses, soon), the native builds, then enable for everyone (no internal-users step, founder decision). Limits: address validation and Mapbox were not exercised (no keys), and no physical device or push was used.

**September 24 Ballot build track (separate from stream integration):** the founder asked for a plan, design and build, with every screen matching the [Ballot canvas](https://claude.ai/artifact/KCuXBiAYYaX13gpoqUCdmq). The [implementation plan](ballot-implementation-plan-2026-09-24.md) corrects the guide's contracts against the code on master `6688fd84` and sets the canvas-matching rules. Branch `claude/blissful-dijkstra-n8r31h` holds the P0 slices, all behind the `ballot_p0` flag (off): backend (`ee386d83`, `2db2d532`), web (`73323bfa`, `a48bdd00`), iOS (`635e3fe3`, `7eb6fac8`) and Android (`e12a73d7`, `bdb43226`). The canvas (version 10) carries the proposed P0 boards for approval, including the animated "P0: the peel, told" and the after-the-election design (plan §11 item 7), approved and built September 24: the card stays through certification ("Counting", then "Certified") with official results links and no counts, Washington's November 2, 2027 general brings it back July 5, 2027, and the Civic page has a year-round "Your governments" row on web, iOS and Android. Verified here: backend Jest 6319 passed, privacy and migration gates, web Jest 1911 passed and the type gate, web screenshots against the rendered boards (cards 0.00–0.02% of pixels differ; governments finished frame 0.09%), and a web end-to-end run on a local stack with the real backend and app ([plan §10.1](ballot-implementation-plan-2026-09-24.md#101-web-end-to-end-run-september-24)). That run found and fixed the governments sheet rendering under the app shell. A later pass fixed the existing Civic page, which shows the election Ballot provides: web showed the day before and iOS a blank date tile (`4daeea8f`), and the past election read "0 days away" the week after it (`b6630803`). The warning-well calendar icons now stay 18px (`c5d69bcf`). Ballot now covers the seven states that mail every voter a ballot (California, Colorado, Hawaii, Nevada, Oregon, Utah and Vermont) as well as Washington, at the founder's request. It's the same card with each state's dates, rules and official links (plan §5.1.1), found by search and pending release check 1. Washington's mailing wording changed for the USPS postmark rule that took effect December 24, 2025. iOS and Android can't be built in this container (no Xcode; the proxy refuses dl.google.com, which serves Google Maven), but PR CI run 1518 on `c7cba932` compiled both and passed their unit and snapshot tests, the iOS simulator tests and the Android emulator tests. Open as [WangPantopus/skinny-pantopus#429](https://github.com/WangPantopus/skinny-pantopus/pull/429). Next: founder approval of plan §11 (items 1–7), green CI on the latest head, then release checks 1–2 and device passes. Limits: the container cannot reach sos.wa.gov, vote.gov, clark.wa.gov, the Census geocoder or dl.google.com, and has no Xcode. No stream ownership, acceptance count or merge state changes.

...
A bounded repair on `codex/landlord-existing-flow-fixes` checks the invitation
...
The active worktree remains `/private/tmp/pantopus-home-permission-boundaries`,
now on that follow-up branch. The Home branch remains at `e6e3c65f8` remotely;
its integration CI 34788091570 has passed every completed check, with Android
lint/test/assemble still running at the latest observation. Do not cancel that
run by pushing this follow-up directly onto #32. No screen or migration changed.

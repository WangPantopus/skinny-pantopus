...
**September 24 — `/start` deep link and address coordinates (web; branch `claude/brave-planck-fkmzrs`, fix `48ec7447`):** `/api/geo/autocomplete` sends each suggestion's `center` as GeoJSON `[lng, lat]`. `backend/tests/unit/geoRoutes.test.js` pins that shape, and the iOS and Android decoders read it as an array. The web `GeoSuggestion` type and its readers expected `{lat, lng}`. Reproduced in real Chromium through the real Next proxy and backend geo route, with the Mapbox provider stubbed to the reported Camas suggestion and `/api/public/place` canned. `/start?address=…` requested autocomplete but never requested the preview, so it stayed on the hero. After picking a suggestion the preview rendered, but **Continue** showed "Your browser could not keep this preview" and stored no draft. Repair: the type is corrected. `suggestionCoordinates()` in `place/pendingPlace.ts` reads `[lng, lat]` and `{lat, lng}` within the draft's bounds; the existing `entryContinuity` harness and unlisted fixture send the object shape. `StartFunnel` and `place/AddressAutocomplete` use the new reader. The corrected type also flagged the same misread in the geocode fallbacks of `LocationPicker` and `magic-task-v2/QuickModifiers`; those are repaired in place. Backend, native and design are unchanged. The existing `startFunnel` fixture now uses the wire shape, and two of its tests fail on the old code. The change merges cleanly with open #429, which also edits `StartFunnel`. Verification after the fix: the deep link requests and renders the preview. Pick → Continue reaches `/register?redirectTo=/app/place?preview=…` with the draft at 45.58716, −122.39966. Web Jest (122 suites, 1,893 tests), the typecheck gate (0 errors), lint (0 errors) and the production build pass. The existing `entryContinuity` Playwright spec and CI's `identity-firewall` Playwright spec pass. An uncommitted component check of the four fallback paths passes; the same check fails 4/4 on the old code. Limits: no live Mapbox or preview data providers. The gig-location fallbacks now perform their designed write-back of user-asserted coordinates to a Home that has none, and the backend guards verified coordinates, but no signed-in real-stack run covered them. No hosted deployment, branch CI run or installed native check. No acceptance row changes. The harness and screenshots were session-local and not retained. Next: the same misread remains in three untyped web readers, queued as a separate task: the home edit "Geocode" button, `feed/PostLocationPicker` and the `gig-v2-create/QuickModifiersV2` fallbacks.

...
**Active native candidate — PR #35:** see [the current WIP report](home-native-history-wip-2026-09-13.md) and [accepted iOS reader evidence](home-ios-residency-review-history-2026-09-13.md). Primary integration remains on `codex/home-permission-boundaries` at `f14989637`; this separate branch is unfinished. Both installed readers and Android optimized Release codecs pass; complete the separate fresh native cycles before integration. The populated reader fixture is exactly cleaned. Root now owns the fresh zero-command iOS cycle; Android is stopped with data retained. See [Android reader evidence](home-android-residency-review-history-2026-09-13.md). The fresh iOS submission phase passes; finish rejection/recovery, resubmission, independent approval, current access/removal and departed access/history next. All four PRs #32/#34/#35/#36 are open drafts. Reader/product source `9350896c4` passes all 16 checks in CI 34768705945.
...
R02 is committed/pushed at `d3c3e0fac3b90e7e0d468ba421cffd5bb0e9475c`,
with all 16 checks green in [CI 34744758908](https://github.com/WangPantopus/skinny-pantopus/actions/runs/34744758908).
Its atomic older-client repair passes populated upgrade, 53 actual HTTP/SDK
calls, ten observed lock races and all 50 raw plus 50 generated H07/R02 SQL
contracts. The old response shape, legitimate membership and protected originals
are preserved. Legacy clients still have no immutable UUID/address-review/cancel
protocol; notices are best effort and saving requests no postage.

The later [backend removal repair](home-member-removal-recovery-backend-2026-09-13.md)
and [browser recovery](home-browser-member-removal-recovery-2026-09-13.md) now pass
actual HTTP/SDK/SQL and Chrome acceptance. One protected original recovers a lost
reply without repeated removal/audit writes; unseen cancellation, changed review,
current authority and history after self-leave pass. Browser r2 finishes five
...
originals therefore require a fresh review after this upgrade; completed receipts
remain historical. Shipped re-invitation refuses ended membership. Do not claim
renewal or protection for legacy clients against a future legitimate nonterminal
membership. Self-leave notices remain best effort; a lost RPC reply can prevent
notice admission. These limits are tested and recorded in the backend report.
...
R03 backend and browser recovery have independently verified durable source,

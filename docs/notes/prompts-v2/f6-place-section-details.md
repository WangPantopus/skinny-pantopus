...
EDGE CASES: A 1%-a-year zone reads "Zone AE — 1% chance of flooding each year (about 1 in 4 over 30 years)". The alert reading reads "AQI 118 · Unhealthy for Sensitive Groups · PM2.5" (observed 4:00 PM). An air reading older than 3 hours keeps its value and shows its age (stale air row); it is never blanked. One layer missing keeps all four rows. No seeded voter rule: the block is absent, not an empty card. A saved place (PLACE B) cannot open this page today; flag it on Notes. Slow load: WarmingSkeleton after 1s. A member without edit rights sees everything; "Remind me" and the done line are personal.
...
- Saved-place access: PLACE B cannot open this page, yet flow-13 (the T1 mover civic path), the place file voter row at T1 and the Foundations 00c-08 crop all assume it can; decide the T1 destination for all three. Proposed: for a saved place, the place file's voter row opens the DateSheet in view-seeded voter mode, which carries the same two method lines and the VoteWA row, until Civic accepts saved places. Also decide where a saved-place viewer's compare-strip tap goes.

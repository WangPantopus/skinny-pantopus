...
ATTACH: the place file (Dates section with a missing row and a done row), Today with the 14-day strip and the pickup card, the provenance sheet's report step, and exports of the Foundations boards 00a–00d. In every phone frame, draw the host dimmed behind the sheet exactly as the screenshot shows it.
...
- Lease ends (HOME A): Wed 31 Mar 2027.
  ...
  - Remind me is 30 days. Ticks: 'Sat 30 Jan 2027 · 30 days before', 'Sun 28 Feb 2027 · Day before', 'Mon 1 Mar 2027 · Day of'.
...
- Lease ends (PLACE B, Jordan): Wed 31 Mar 2027, 30 days' notice, Notice deadline Mon 1 Mar 2027, 30 days.
...
- 'Remind me' (ReminderLeadControl), 44pt tall:
  - Date-sheet variant: Off · 60 days · 30 days · 7 days · 1 day. Lease variant, anchored to the Notice deadline: Off · 60 · 30 · 14 · 7 · 1, with 30 selected. Draw both exactly as the Foundations board shows them. If labels don't fit one row, wrap to a 3 × 2 grid.
...
- Mini timeline (Foundations anatomy), not to scale:
  ...
  - Under each tick, the Foundations labels: the date with year ('Sat 30 Jan 2027'), then the role ('30 days before' / 'Day before' / 'Day of').
...
DateSheet (create, view-mine, view-seeded, conditional seeded, bill mode), KindGlyph (tile and row), ReminderLeadControl (date-sheet and lease-notice variants as drawn on the Foundations board, including Off and the lease 30 days), DateRow (deadline, conditional rule, done, money row, pickup, holiday moved), ProvenanceMark (S and M; read-from-your-photo in bill mode), SourceCaption, ScopeChip (footer sentence form), ChoiceChip (visibility, notice length, weekday, frequency, next recycling, voter plan), InlineUndo, InlineErrorRow (save failed, load failed, remove failed), OfflineNotice (form offline, queued writes), LockedActionRow (names who can act), TextActionRow (outbound), WarmingSkeleton (row skeleton), ProvenanceSheet (opened from another sheet), PushCopy (date reminder, Notes only).
...
- The timeline speaks one sentence: 'Reminders on Sat 30 Jan, Sun 28 Feb and Mon 1 Mar.' (bill: 'Reminders on Thu 22 Oct and Fri 23 Oct.'). At AX sizes it becomes a list of rows with the same words.
...
ARTBOARDS
1. x-date-sheet · ios · 01-create-lease-dense · light — HOME A, Lease ends, 30 days' notice, linked Notice deadline row, 30 selected, three ticks with the event line, 'Visible to' with Only you selected, footer 'Only you will see this.' Small inset labelled 'three taps' showing the iOS date path.
...
22. x-date-sheet · ios · 22-loading-from-reminder · light — dated Sat 30 Jan 2027; the highlighted row ('in 30 days · Mon 1 Mar 2027') and the sheet with a row skeleton.
...
38. x-date-sheet · Notes —
  - Reminder push, drawn as the Foundations PushCopy date-reminder tray preview: 'Lease notice due in 30 days' / 'Tell your landlord by Mon 1 Mar', action 'Done', 7:00 AM morning slot, Active level on iOS and DEFAULT importance on Android.

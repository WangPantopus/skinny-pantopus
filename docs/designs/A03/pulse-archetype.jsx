// Pantopus — Pulse (feed) tab archetype
// 3 frames: Populated · Empty · Loading
...
function ChipRow({ activeKey = 'all', skeleton = false }) {
  return (
    <div style={{
      ...
      {INTENTS.map((it, i) => {
        if (skeleton) {
          return (
            <div key={i} style={{
              height:28, width: i === 0 ? 50 : (60 + (i*8)%40), borderRadius:9999,
  ...
  return (
    <Phone>
      ...
      <div style={{
        ...
        }}>Be the first to post. Ask, recommend, or announce something local.</p>
        <button style={{
...
// ═══════════════════════════════════════════════════════════════
function ShimmerBlock({ w, h, r = 6, style = {} }) {
  return (
    <div style={{
...
function SkeletonCard({ withTitle = false }) {
  return (
    <div style={{
      background:PU.surface, border:`1px solid ${PU.border}`,
      borderRadius:16, padding:12,
      ...
      <div style={{display:'flex', alignItems:'center', gap:9, marginBottom:10}}>
        ...
        <div style={{flex:1, display:'flex', flexDirection:'column', gap:5}}>
          <ShimmerBlock w={110} h={10}/>
          <ShimmerBlock w={70} h={8}/>
        </div>
        <ShimmerBlock w={42} h={16} r={9999}/>
      </div>
      {withTitle && (
        <div style={{marginBottom:6}}>
          <ShimmerBlock w="60%" h={11}/>
        </div>
      )}
      {/* body lines */}
      <div style={{display:'flex', flexDirection:'column', gap:5}}>
        <ShimmerBlock w="100%" h={9}/>
        <ShimmerBlock w="86%" h={9}/>
      </div>
      {/* reactions */}
      <div style={{display:'flex', gap:14, marginTop:12}}>
        <ShimmerBlock w={56} h={10}/>
        <ShimmerBlock w={56} h={10}/>
        <div style={{flex:1}}/>
        <ShimmerBlock w={42} h={10}/>
      </div>
    </div>
  );
}

function FramePulseLoading() {
  return (
    <Phone>
      <TopBar title="Pulse"/>
      <ChipRow skeleton/>
...
Object.assign(window, { FramePulsePopulated, FramePulseEmpty, FramePulseLoading });

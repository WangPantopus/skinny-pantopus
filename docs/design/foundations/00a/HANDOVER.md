...
Nine components drawn on the Foundations 00a specimen board, as published to the Pantopus design system on 22 September 2026 (version 12). This pack is the same content as files, so it can be committed.

**ProvenanceMark · SourceCaption · ScopeChip · FreshnessLine · OfflineNotice · StatusChip · KindGlyph · AddressChip · ChoiceChip.** The board is complete: 49 artboards, 387 named specimens, every variant at every state, platform, text size and theme.

This revision adds AddressChip — drawn in full across variants, states, in-context crops, large text and greyscale — and gives ChoiceChip the two passed-lead states that 00d's NotificationAsk time chips and ReminderLeadControl lead segments both collapse into.
...
The paths mirror the design system's own layout, so the tree can be copied straight over `project/`. `foundations-00a.md` was published alongside the nine cards; it carries the rules they share, the open decisions and the token-correction note. It is included here so a commit of "what we published" is complete — drop it if you only want the components.
...
### AddressChip
`frontend/apps/web/src/components/archetypes/place/` — it belongs with the address surfaces, not the generic primitives.

- **Props:** `variant`: `held` | `recovery-row` | `duplicate` | `sender`; `state`: `default` | `selected` | `expired`; plus the hold copy and a duplicate's saved name.
...
- **Never a map, never coordinates.** The chip is a label. Its job is to show the address the server already holds so nobody retypes it between steps.
- **The scope sentence belongs to the component, not the page.** Held, recovery and duplicate chips carry "Only you will see this." 8pt below — the same sentence ScopeChip draws in 00a-03, reused exactly. A `sender` label describes someone else's place, so it carries none; the sentence moves under the recipient's own address field, where it is true. This is the single exception to the scope rule across all nine components, so it should be enforced by the type rather than remembered.
- **Two contrast facts that have to survive the trip:** on a selected `color-primary-50` fill the city goes to `app-text` — keeping `app-text-secondary` there is 3.69:1 in dark. And in dark the chip has no border at all, because `app-border` on dark `app-surface-raised` is 1.00:1; the pin and the text carry it. The selected outline stays, because it means something.
- **Expiry is not an error.** `app-surface-sunken`, `app-text-strong`, no warning or error hue, and the action reopens search prefilled with the address. A hold running out is not a failure — same logic as OfflineNotice.
- **No truncation, ever.** Street and city wrap to as many lines as they need. At large text the pill becomes a radius-lg rectangle, the pin grows to 32, and the action moves under the address as a full-width button.
- **Open:** whether the sender label may drop the scope sentence — board entry 08.D, unanswered. It is exactly what makes this the one exception.
...
- **The two passed-lead states, from 00d.** A lead whose date is before today has passed, and takes exactly one of two treatments — never a third, never both:
  - *Passed, never chosen* — ordinary disabled. `app-text-muted` label and outline, no check, still focusable, read as "N days, unavailable, N days before was <date>, which has passed." The reason is also written out in readable text: a line 8pt below the control, bodySmall 14/20 `app-text-secondary`, naming only the never-chosen passed options. With none, there is no line.
  - *Chosen, then passed* — it was selected and then the date moved. `color-primary-50` fill, 1.5px `color-primary-700` outline, **no check**, label at bodySmall 14/20 weight 400 in `app-text`, and the word "passed" under it in caption 12/16 `app-text`. Dark: `color-primary-900` fill, 1.5px `color-primary-400` outline, both lines in `app-text`. Read as "N days, selected, passed. N days before was <date>, which has passed." It keeps this treatment until another lead is picked.
  - **Activating a passed option does not select it** — it shows the explanation and the suggestion. The suggestion is always the longest lead still ahead, and nothing is saved until the person taps it or picks a live option. So the displayed value and the saved value diverge here; that is component state, not page state, and it is the part most likely to be lost in translation.
  - **Height consequence, and it is not cosmetic:** chosen-then-passed is **44 tall against ChoiceChip's 32**, because two lines of 14/20 and 12/16 need 36. A group holding one sits taller than its neighbours, so any row sizing chips by a constant breaks. Size the group by its tallest chip.
  - **Two sizes doing one job.** ChoiceChip's own offline reason is a caption at 12/16; 00d's passed explanation is bodySmall at 14/20. Both sit under the same control and say the same kind of thing. The board records the conflict rather than silently picking — worth settling before either ships.
...
| AddressChip | Chip (identity variants), SearchInput | SearchInput takes an address the person types; AddressChip shows one the server holds |

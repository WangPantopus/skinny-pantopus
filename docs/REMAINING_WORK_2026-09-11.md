...
- [ ] R05 **Verify:** PR #38 repairs demonstrated lease approval/date/admission, end/move-out, tenant status/cancel and request-submission defects using existing records and designs. End source1f526de0d passes eight CI checks/three unchanged skips; legacy protected-removal → end → fresh-admission passes. Native Leave Home controllers are unchanged and accepted evidence is reused. Current request source passes 126 backend/37 rendered tests, SQL lifecycle/rollback/lint, actual submission-race and response-loss/expired-lease browser journeys, and 12 admission/five end HTTP/SQL compatibility checks. It fixes the old fixture omission that failed CI08e0b6177; current CI is pending. Next: stale reads/actions across Home/account/background changes and unseen/queued submission identity, then remaining installed verification consumers. No table or second migration added. Provider delivery, combined adoption and rollout remain open. See [current evidence and limits](VERIFICATION_FIRST_2026-09-13.md#existing-request-submission-and-response-loss-follow-up).

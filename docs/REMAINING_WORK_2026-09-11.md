...
- [ ] R05 **Verify:** Lease creation/change/end, renter/landlord/manager authority and access expiry must remain coherent with ownership and household admission. September 13 actual service/SDK/PostgREST/SQL verification reproduces approval success with null occupancy and a blocked retry, and reactivation retaining an expired access deadline. The follow-up date patch passes eight rendered web/SDK tests, 187 backend tests and seven real HTTP/SQL checks, but stays draft: a separate probe confirms future-start leases grant immediate access even without date edits. Finish membership/lease consistency before integrating this patch; see [the date draft](VERIFICATION_FIRST_2026-09-13.md#existing-lease-date-controls--september-13-draft). Both remain open. The existing invitation service now checks its issuer’s current verified authority before writes; revoked-issuer denial and valid acceptance pass the bounded SQL comparison, with 279 focused service/route tests. Concurrent revocation and atomic lease/occupancy/retry behavior remain open. See [the reuse verification](VERIFICATION_FIRST_2026-09-13.md#existing-landlord-flow-verification--september-13); preserve existing native request forms and web landlord screens.

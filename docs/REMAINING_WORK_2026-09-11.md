...
- [ ] R05 **Verify:** Lease creation/change/end, renter/landlord/manager authority and access expiry must remain coherent with ownership and household admission. The September 13 PR #38 draft repairs reproduced date loss, false approval success, expired reentry bounds and early future access using one transaction over existing records. Local SQL lifecycle/rollback checks, 12 actual HTTP/SQL checks, five multi-session races and 134 service/route checks pass. Existing web layout and native forms are preserved. Ten rendered checks and six actual browser/HTTP/SQL cases now pass after small error-message, canceled-denial and calendar-display repairs. Transaction CI81cf3bef8 passes 8 executed checks/3 skips. Next: actual end probes show independent admin access is removed and a failed deactivation leaves lease ended/access active with blocked retry; repair that existing end boundary. Legacy active records without receipts, installed lease workflows, provider delivery and combined adoption remain unaccepted. See [the transaction evidence and limits](VERIFICATION_FIRST_2026-09-13.md#existing-lease-decision-transaction--september-13-draft).

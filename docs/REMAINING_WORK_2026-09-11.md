...
- [ ] R05 **Verify:** Lease creation/change/end, renter/landlord/manager authority and access expiry must remain coherent with ownership and household admission. September 13 actual service/SDK/PostgREST/SQL verification reproduces approval success with null occupancy and a blocked retry, and reactivation retaining an expired access deadline. Both remain open. The existing invitation service now checks its issuer’s current verified authority before writes; revoked-issuer denial and valid acceptance pass the bounded SQL comparison, with 279 focused service/route tests. Concurrent revocation and atomic lease/occupancy/retry behavior remain open. See [the reuse verification](VERIFICATION_FIRST_2026-09-13.md#existing-landlord-flow-verification--september-13); preserve existing native request forms and web landlord screens.

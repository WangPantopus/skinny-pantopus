...
- [ ] R05 **Verify:** Lease creation/change/end, renter/landlord/manager authority and access expiry must remain coherent with ownership and household admission. PR #38 repairs demonstrated approval/date/admission and end/move-out defects using existing records and UI. End source1f526de0d passes eight CI checks/three unchanged skips; four actual legacy protected-removal → end → fresh-admission groups also pass. Both native Leave Home controllers are unchanged and accepted evidence is reused. The next follow-up connects the existing tenant status/cancel controls to missing routes, projects only the caller's own lease, and removes the false future/full-access claim without design changes. Current 128 backend tests, 13 rendered tests, six tenant browser/HTTP/SQL groups, two cancellation/approval races, real SQL rollback and fresh type/lint checks pass; follow-up CI is pending. Next: existing request submission and uncertain-reply recovery, remaining native verification consumers, and stale-response/account boundaries. Provider delivery, combined adoption and hosted rollout remain open. See [current evidence and limits](VERIFICATION_FIRST_2026-09-13.md#existing-tenant-status-and-cancellation-follow-up).

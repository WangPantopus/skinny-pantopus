...
Native reviewer history is accepted and integrated through PR #35 into the primary Home branch (merge `f1a92f45a`, exact source `8cfcbf2b2`, passing CI 34775715928). Both installed readers and both fresh native cycles pass with preserved evidence and exact cleanup. PR #36 current-claims local acceptance now passes HTTP/SDK, both browser consumers, both installed native readers, actual decision-driven empty queues and exact cleanup. Final exact-head CI/integration remains next. See [current-claims evidence and limits](home-current-claims-wip-2026-09-13.md), then continue the ordered backlog. PRs #32/#34/#36 remain drafts. Counts remain 8 closed and 72 partial/open.

Updated September 13, 2026 after current-claims local acceptance.
...
- [ ] G03 **Integrate:** Reconcile the accepted current-claims candidate’s 50 Home / 21 payment / 59 distinct combined migration versions and dependency order (12 byte-identical shared versions, zero collisions at this checkpoint). The source includes Task defaults `20260912050000` R02 compatibility `20260912060000` removal recovery `20260913010000` reviewer history `20260913020000` and current claims `20260913030000`. The latest 104-contract Home proof applies Home prerequisites only and does not complete combined paid/Home replay or adoption. The reserved tip migration is not yet implemented and will change the final set.

...
- [ ] R05 **Verify:** Lease creation/change/end, renter/landlord/manager authority and access expiry must remain coherent with ownership and household admission. PR #38 repairs demonstrated approval/date/admission and end/move-out defects using existing records and UI. Lease termination is atomic, preserves independent/replaced access, and exposes retryable errors and existing history. Co-resident self-departure preserves the primary lease and reuses accepted Home removal policy. Current 125 service/route tests, ten rendered web tests, real SQL/rollback contracts, five end HTTP/SQL comparisons, co-resident HTTP/SQL comparisons, seven SQL races and three browser end cases pass. Earlier admission and six browser cases retain their source-specific evidence. Fresh standalone TypeScript passes after correcting test-only options that failed CI695a1cfb3. End follow-up CI is pending. Next: explicit legacy membership review/removal → lease-end retry, then reconcile existing native Home move-out callers before any UI work. Provider delivery, combined adoption and hosted rollout remain open. See [the current evidence and limits](VERIFICATION_FIRST_2026-09-13.md#existing-lease-end-and-move-out-follow-up).

...
- [ ] P04 **Fix/Verify:** Finish started-work, no-show, cancellation-fee and completion/reopen policies, immutable displayed terms and exact fee execution. Existing zero-fee unstarted-stop recovery is a bounded completed source milestone. Start Work backend follow-up `599de1586` passes bounded local HTTP/SQL and regressions; native stale/invalid-result and installed acceptance remain open. See [Stream1 evidence and limits](workstreams/01-gigs-payments.md).

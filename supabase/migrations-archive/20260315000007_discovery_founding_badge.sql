-- Update find_businesses_nearby to return founding_badge and founding_benefit_expires_at
-- for discovery scoring, and accept optional entity_type filter.
...
CREATE OR REPLACE FUNCTION "public"."find_businesses_nearby"(
  ...
  "founding_benefit_expires_at" timestamp with time zone
)
  LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    -- Founding badge fields
    bp.founding_badge,
    bp.founding_benefit_expires_at
  ...
  WHERE u.account_type = 'business'
    ...
    -- Entity type filter
    AND (p_entity_types IS NULL OR bp.business_type = ANY(p_entity_types))
...
ALTER FUNCTION "public"."find_businesses_nearby"(
  double precision, double precision, integer, uuid, text[], numeric, integer, text[]
) OWNER TO "postgres";

GRANT ALL ON FUNCTION "public"."find_businesses_nearby"(double precision, double precision, integer, uuid, text[], numeric, integer, text[]) TO "anon";
GRANT ALL ON FUNCTION "public"."find_businesses_nearby"(double precision, double precision, integer, uuid, text[], numeric, integer, text[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."find_businesses_nearby"(double precision, double precision, integer, uuid, text[], numeric, integer, text[]) TO "service_role";

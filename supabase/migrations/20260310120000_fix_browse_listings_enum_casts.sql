-- Fix: compare enum columns to text params via ::text (operator does not exist: listing_category = text)
...
BEGIN
  ...
  SELECT
    ...
    l.latitude,
    l.longitude,
    l.location_precision::text,
    l.visibility_scope::text,
    l.meetup_preference,
    ...
    l.created_at,
    l.updated_at,
    ST_Distance(ST_SetSRID(ST_MakePoint(l.longitude, l.latitude), 4326)::geography, v_ref)::double precision AS distance_meters
  FROM "Listing" l
  WHERE l.status = 'active'
    AND l.archived_at IS NULL
    AND l.latitude  IS NOT NULL
    AND l.longitude IS NOT NULL
    AND l.latitude  >= p_south AND l.latitude  <= p_north
    AND l.longitude >= p_west  AND l.longitude <= p_east
    ...
    AND (
      ...
      OR ST_Distance(ST_SetSRID(ST_MakePoint(l.longitude, l.latitude), 4326)::geography, v_ref) > p_cursor_distance
      OR (ST_Distance(ST_SetSRID(ST_MakePoint(l.longitude, l.latitude), 4326)::geography, v_ref) = p_cursor_distance AND l.id > p_cursor_id)
    )

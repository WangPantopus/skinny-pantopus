...
@HiltViewModel
class HomeResidencyHistoryViewModel
    @Inject
    constructor(private val factory: HomeResidencyHistoryFactory) : ViewModel() {
        ...
        fun nextPage() {
            ...
            if (current.working || !current.confirmed || current.failure != null) return
            if (target?.reference != null) return
        ...
        private fun request(
            ...
            previous: List<HomeResidencyHistoryItem>,
        ) {
            ...
            scope.launch {
                try {
                    ...
                    target.reference?.let { reference ->
                        if (reference.homeId != target.homeId || reference.actorId != identity.actorId) {

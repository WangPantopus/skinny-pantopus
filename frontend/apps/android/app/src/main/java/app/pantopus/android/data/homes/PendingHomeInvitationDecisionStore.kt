...
interface PendingHomeInvitationDecisionStore {
    suspend fun read(scope: HomeCreationScope): PendingHomeInvitationDecision?
...
@Singleton
class PersistentPendingHomeInvitationDecisionStore
    @Inject
    constructor(
        @ApplicationContext private val context: Context,
        moshi: Moshi,
    ) : PendingHomeInvitationDecisionStore {
        private val adapter = moshi.adapter(PendingHomeInvitationDecision::class.java).failOnUnknown()
        private val codec = HomeInvitationDecisionCodec(moshi)
        private val lock = Mutex()
        private var preferences: SharedPreferences? = null
        private val uncertainValues = mutableMapOf<String, String?>()

        override suspend fun read(scope: HomeCreationScope): PendingHomeInvitationDecision? = locked { read(it, scope) }
        ...
        override suspend fun replace(
            ...
            locked { prefs ->
                ...
                if (!edit.commit()) {
                    ...
                    error("Invitation decision recovery could not be saved. Retry before starting another request.")
                }
        ...
        private fun read(
            ...
            ) { "The saved invitation decision could not be read. Keep it and retry recovery." }
            return value
        }
                ...
                lock.withLock {
                    try {
                        ...
                        throw IllegalStateException(
                            "Invitation decision recovery storage is unavailable. Keep the original and retry.",
                            error,
                        )
                    } catch (error: GeneralSecurityException) {
                        throw IllegalStateException("Private invitation decision recovery storage could not be opened.", error)
                    } catch (error: SecurityException) {
                        throw IllegalStateException("Private invitation decision recovery storage could not be accessed.", error)
                    } catch (error: JsonDataException) {
                        throw IllegalStateException("The saved invitation decision could not be read. Keep it and retry recovery.", error)
                    }
                }
            }
        ...
        private fun open(): SharedPreferences {
            ...
            return EncryptedSharedPreferences.create(
                context,
                "private_home_invitation_decision_v1",
                master,

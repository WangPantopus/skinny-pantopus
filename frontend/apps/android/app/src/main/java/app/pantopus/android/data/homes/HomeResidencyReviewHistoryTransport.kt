...
class APIHomeResidencyReviewHistoryTransport(
    ...
    override suspend fun session(scope: HomeCreationScope): HomeResidencyHistorySession =
        checked {
            check(scope.isValid())
        ...
        val kind =
            when {
                response.code() == HttpURLConnection.HTTP_CONFLICT && code == "SESSION_SCOPE_CHANGED" ->
                    HomeResidencyHistoryFailureKind.SessionChanged
                response.code() == HttpURLConnection.HTTP_FORBIDDEN && code == "MEMBERS_MANAGE_REQUIRED" ->
                    HomeResidencyHistoryFailureKind.Forbidden
                response.code() == HttpURLConnection.HTTP_NOT_FOUND && code == "HOME_NOT_FOUND" ->
                    HomeResidencyHistoryFailureKind.MissingHome
                response.code() == HttpURLConnection.HTTP_NOT_FOUND && code == "RESIDENCY_HISTORY_NOT_FOUND" ->
                    HomeResidencyHistoryFailureKind.MissingDecision
                response.code() == HttpURLConnection.HTTP_BAD_REQUEST && code == "RESIDENCY_HISTORY_CURSOR_INVALID" ->
                    HomeResidencyHistoryFailureKind.InvalidCursor

...
class HomeResidencyHistoryTransportTest {
    ...
    @Test fun fresh_session_list_and_detail_use_only_get_and_no_store() =
        runTest {
            server.enqueue(
    ...
    @Test fun denied_missing_invalid_cursor_and_session_failures_are_not_empty() =
        runTest {
    ...
    @Test fun malformed_and_foreign_success_never_establish_authorized_history() =
        runTest {
            for (body in listOf(
                "[]",
                "null",
                "{}",
                f.page().replace(f.actor, f.id(88)),
                f.page().replace(f.scope.sessionScope, "b".repeat(64)),
            )) {
                server.enqueue(MockResponse().setBody(body))
                assertEquals(
                    HomeResidencyHistoryFailureKind.Unavailable,
                    (runCatching { transport.list(f.home, f.scope, null) }.exceptionOrNull() as HomeResidencyHistoryFailure).kind,
                )
            }
    ...
    @Test fun pagination_transmits_exact_validated_cursor_and_refuses_forged_scope_locally() =
        runTest {

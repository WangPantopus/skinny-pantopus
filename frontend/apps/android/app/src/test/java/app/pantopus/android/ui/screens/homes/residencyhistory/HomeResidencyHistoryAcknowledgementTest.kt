...
class HomeResidencyHistoryAcknowledgementTest {
        ...
        object : HomeResidencyReviewTransport {
            override suspend fun read(
                ...
                HomeResidencyCurrentReview(
                    f.home,
                    f.actor,
                    f.scope.sessionScope,
                    mapOf("id" to claimId, "user_id" to f.id(5), "status" to "verified", "review_token" to "b".repeat(64)),
                    null,
                )
            ...
            override suspend fun decide(
                ...
                error(
                    "History link must never submit a command",
                )
        }
    ...
    @Before fun setup() {
        ...
        val draft =
            PendingHomeResidencyReview(
                ...
                f.id(3),
                f.id(201),
                HomeResidencyDecision.Approve,
                codec.request(f.id(201), "b".repeat(64), HomeResidencyDecision.Approve, HomeResidencyReviewRole.Member, ""),
            )
        val receipt =
            mapOf(
                "id" to f.id(200), "home_id" to f.home, "claim_id" to f.id(3), "actor_id" to f.actor,
    ...
    @Test fun successful_actual_acknowledgement_exposes_only_real_receipt_id_and_retires_on_pause() =
        runTest {
            vm.acknowledge()
            assertNull(saved)
    ...
    @Test fun failed_clear_retains_original_without_history_navigation() =
        runTest {
            failClear = true
            vm.acknowledge()
            assertNotNull(saved)
    ...
    @Test fun late_clear_after_close_cannot_publish_post_ack_reference() =
        runTest {

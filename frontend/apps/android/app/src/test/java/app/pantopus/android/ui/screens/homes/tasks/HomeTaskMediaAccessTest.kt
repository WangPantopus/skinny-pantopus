...
class HomeTaskMediaAccessTest {
    ...
    @Test fun exact_read_only_list_and_bytes_never_mutate() =
        runTest {
    ...
    @Test fun both_task_and_media_permissions_are_required_for_writes() =
        runTest {
            val access = f.access(backgroundScope)
    ...
    @Test fun foreign_duplicate_or_unsafe_legacy_metadata_is_denied() =
        runTest {
            val access = f.access(backgroundScope)
            for (records in listOf(
                listOf(f.record.copy(homeId = f.task)),
                listOf(f.record.copy(taskId = f.home)),
                listOf(f.record, f.record),
                listOf(f.record.copy(state = "legacy")),
            )) {
    ...
    @Test fun stored_only_session_replacement_denies_before_any_request() =
        runTest {
            val access = f.access(backgroundScope)
    ...
    @Test fun changed_server_scope_cannot_upload() =
        runTest {
            val access = f.access(backgroundScope)
            access.list()
            coEvery { f.tasks.getHomeTask(any(), any(), any()) } returns
    ...
    @Test fun upload_uses_immutable_bytes_safe_name_and_exact_current_receipt() =
        runTest {
            val access = f.access(backgroundScope)
    ...
    @Test fun wrong_upload_receipt_cannot_confirm_success() =
        runTest {
            val access = f.access(backgroundScope)
            for (wrong in listOf(
                f.record.copy(id = f.home),
                f.record.copy(uploadedBy = "other"),
                f.record.copy(fileName = "other.txt"),
                f.record.copy(fileSize = 1),
                f.record.copy(state = "retired", available = false),
            )) {
    ...
    @Test fun concurrent_retirement_after_download_hides_and_erases_bytes() =
        runTest {
    ...
    @Test fun current_task_denial_after_download_hides_and_erases_bytes() =
        runTest {
            val delivered = f.bytes.copyOf()
            coEvery { f.repository.download(any(), any(), any()) } answers {
    ...
    @Test fun removal_needs_exact_complete_cleanup_receipt() =
        runTest {
            val access = f.access(backgroundScope)
    ...
    @Test fun other_uploaders_removed_record_may_be_absent_after_exact_receipt() =
        runTest {
    ...
    @Test fun a_second_mutation_cannot_overtake_a_suspended_upload() =
        runTest {

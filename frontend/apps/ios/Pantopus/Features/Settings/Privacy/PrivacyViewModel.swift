...
//  privacy, a "Your data" card of leading-icon action rows and a
//  detached destructive Delete row. A dark `StealthBanner` rides above
//  the first card in the stealth frame.
//
...
//  that look saved but reset, beside sample addresses and a made-up
//  "Last updated" date, misled people (UX inventory S3-29). Profile
//  visibility stays editable in Edit profile. Bring the cards back only
//  with real backend fields. Copy is the parity contract, mirrored
//  word-for-word on Android.
//
//  Two variant frames cover the design parity audit:
//    `.populated` — everyday defaults
//    `.stealth`   — the stealth banner
//
...
public final class PrivacySettingsViewModel: GroupedListDataSource {
    ...
    /// No footer: there is no real "last updated" time to show.
    ...
    public func toggleRow(_ rowId: String, isOn: Bool) async {
        ...
        guard rowId == "appLockToggle" else { return }

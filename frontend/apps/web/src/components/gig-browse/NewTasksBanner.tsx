...
export default function NewTasksBanner({ count, onTap, onDismiss }: NewTasksBannerProps) {
  ...
  return (
    ...
    >
      <button
        onClick={onTap}
        aria-label={`Load ${count} new task${count !== 1 ? 's' : ''}`}
      ...
      >
        <span>
          {count} new task{count !== 1 ? 's' : ''} posted
        </span>
        <span className="opacity-75">— Tap to refresh</span>
      </button>
      <button
        onClick={(e) => {
          e.stopPropagation();
        ...
        aria-label="Dismiss new tasks notification"
      >

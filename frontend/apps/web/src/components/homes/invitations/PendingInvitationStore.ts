...
import { validInvitationDraft, type InvitationDraft } from './invitationDecisionModel';

export interface InvitationSnapshot { draft: InvitationDraft; revision: string }
interface SealedDraft { version: 1; revision: string; iv: ArrayBuffer; ciphertext: ArrayBuffer }
/** Encrypted original invitation token and decision, with atomic compare-and-write across tabs. */
export class PendingInvitationStore {
  ...
  constructor(readonly origin: string, readonly actorId: string) {
    this.key = JSON.stringify(['home-invitation-decision-v1', origin, actorId]);
    this.additionalData = new TextEncoder().encode(`pantopus-home-invitation-decision-v1:${this.key}`);
  }

  async load(): Promise<InvitationSnapshot | null> {
    ...
    try {
      ...
      const key = await readTaskRecoveryValue<CryptoKey>(db, 'keys', 'home-invitation-decision-v1');
      ...
      const draft = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)) as InvitationDraft;
      if (!validInvitationDraft(draft, this.origin, this.actorId)) throw new Error('Invalid saved request');
      return { draft, revision: sealed.revision };
    } catch {
      // A corrupt command is never treated as an empty slot or silently erased.
      throw new Error('The original invitation decision could not be read. It has been kept; no replacement request was submitted.');
    }
  }

  async save(draft: InvitationDraft, expected: InvitationSnapshot | null, isCurrent: () => boolean): Promise<InvitationSnapshot> {
    if (!validInvitationDraft(draft, this.origin, this.actorId)) throw new Error('The original invitation decision is invalid.');
    const db = await openTaskRecoveryDatabase();
    const key = await taskRecoveryEncryptionKey(db, 'home-invitation-decision-v1');
  ...
  async clear(expected: InvitationSnapshot, isCurrent: () => boolean): Promise<void> {
    await this.replace(await openTaskRecoveryDatabase(), expected, null, isCurrent);
  }

  private replace(db: IDBDatabase, expected: InvitationSnapshot | null, next: SealedDraft | null,
    isCurrent: () => boolean): Promise<void> {
    return new Promise((resolve, reject) => {
      const transaction = db.transaction('drafts', 'readwrite');
      const store = transaction.objectStore('drafts');
      const request = store.get(this.key);
      let failure = 'The original invitation decision could not be saved. No replacement request was submitted.';
      request.onsuccess = () => {
        let current = false;
        ...
        if (expected ? actual?.revision !== expected.revision : actual !== undefined) {
          failure = 'Another tab changed the saved invitation decision. Reopen this form to recover that request.';
          transaction.abort(); return;
        }
        try {

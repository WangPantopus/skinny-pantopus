...
import { validRemovalDraft, type RemovalDraft } from './removalModel';

export interface RemovalSnapshot { draft: RemovalDraft; revision: string }
interface SealedDraft { version: 1; revision: string; iv: ArrayBuffer; ciphertext: ArrayBuffer }
/** Encrypted removal original and receipt, with atomic compare-and-write across tabs. */
export class PendingRemovalStore {
  private readonly key: string;
  private readonly additionalData: Uint8Array<ArrayBuffer>;

  constructor(readonly origin: string, readonly actorId: string) {
    this.key = JSON.stringify(['home-member-removal-v1', origin, actorId]);
    this.additionalData = new TextEncoder().encode(`pantopus-home-member-removal-v1:${this.key}`);
  }

  async load(): Promise<RemovalSnapshot | null> {
    ...
    try {
      ...
      const key = await readTaskRecoveryValue<CryptoKey>(db, 'keys', 'home-member-removal-v1');
      ...
      const draft = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)) as RemovalDraft;
      if (!validRemovalDraft(draft, this.origin, this.actorId)) throw new Error('Invalid saved request');
      return { draft, revision: sealed.revision };
    } catch {
      // A corrupt command is never treated as an empty slot or silently erased.
      throw new Error('The original member removal could not be read. It has been kept; no replacement request was submitted.');
    }
  }

  async save(draft: RemovalDraft, expected: RemovalSnapshot | null, isCurrent: () => boolean): Promise<RemovalSnapshot> {
    if (!validRemovalDraft(draft, this.origin, this.actorId)) throw new Error('The original member removal is invalid.');
    const db = await openTaskRecoveryDatabase();
    const key = await taskRecoveryEncryptionKey(db, 'home-member-removal-v1');
  ...
  async clear(expected: RemovalSnapshot, isCurrent: () => boolean): Promise<void> {
    await this.replace(await openTaskRecoveryDatabase(), expected, null, isCurrent);
  }

  private replace(db: IDBDatabase, expected: RemovalSnapshot | null, next: SealedDraft | null,
    isCurrent: () => boolean): Promise<void> {
    return new Promise((resolve, reject) => {
      const transaction = db.transaction('drafts', 'readwrite');
      const store = transaction.objectStore('drafts');
      const request = store.get(this.key);
      let failure = 'The original member removal could not be saved. No replacement request was submitted.';
      request.onsuccess = () => {
        let current = false;
        try { current = isCurrent(); } catch { /* A missing lifetime proof cannot authorize a write. */ }
        if (!current) {
          failure = 'This removal page is no longer current. The saved request was kept.';
        ...
        if (expected ? actual?.revision !== expected.revision : actual !== undefined) {
          failure = 'Another tab changed the saved member removal. Reopen this form to recover that request.';
          transaction.abort(); return;
        }
        try {

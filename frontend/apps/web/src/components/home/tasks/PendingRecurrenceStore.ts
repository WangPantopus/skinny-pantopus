...
import { validPendingRecurrence, type PendingRecurrence } from './homeTaskRecurrenceModel';

export interface RecurrenceSnapshot { draft: PendingRecurrence; revision: string }
interface SealedDraft { version: 1; revision: string; iv: ArrayBuffer; ciphertext: ArrayBuffer }
/** Encrypted original schedule command, with atomic compare-and-write across tabs. */
export class PendingRecurrenceStore {
  ...
  constructor(readonly origin: string, readonly actorId: string, readonly homeId: string, readonly taskId: string) {
    this.key = JSON.stringify(['recurrence', origin, actorId, homeId, taskId]);
    this.additionalData = new TextEncoder().encode(`pantopus-task-recurrence-v1:${this.key}`);
  }

  async load(): Promise<RecurrenceSnapshot | null> {
    ...
    try {
      ...
      const key = await readTaskRecoveryValue<CryptoKey>(db, 'keys', 'task-recurrence-v1');
      ...
      const draft = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)) as PendingRecurrence;
      if (!validPendingRecurrence(draft, this.origin, this.actorId, this.homeId, this.taskId)) throw new Error('Invalid saved request');
      return { draft, revision: sealed.revision };
    } catch {
      // A corrupt command is never treated as an empty slot or silently erased.
      throw new Error('The original schedule change could not be read. It has been kept; no new schedule change was submitted.');
    }
  }

  async save(draft: PendingRecurrence, expected: RecurrenceSnapshot | null, isCurrent: () => boolean): Promise<RecurrenceSnapshot> {
    if (!validPendingRecurrence(draft, this.origin, this.actorId, this.homeId, this.taskId)) throw new Error('The original schedule change is invalid.');
    const db = await openTaskRecoveryDatabase();
    const key = await taskRecoveryEncryptionKey(db, 'task-recurrence-v1');
  ...
  async clear(expected: RecurrenceSnapshot, isCurrent: () => boolean): Promise<void> {
    await this.replace(await openTaskRecoveryDatabase(), expected, null, isCurrent);
  }

  private replace(db: IDBDatabase, expected: RecurrenceSnapshot | null, next: SealedDraft | null,
    isCurrent: () => boolean): Promise<void> {
    return new Promise((resolve, reject) => {
      const transaction = db.transaction('drafts', 'readwrite');
      const store = transaction.objectStore('drafts');
      const request = store.get(this.key);
      let failure = 'The original schedule change could not be saved. No replacement request was submitted.';
      request.onsuccess = () => {
        let current = false;
        try { current = isCurrent(); } catch { /* A missing lifetime proof cannot authorize a write. */ }
        if (!current) {
          failure = 'This schedule panel is no longer current. The saved request was kept.';
        ...
        if (expected ? actual?.revision !== expected.revision : actual !== undefined) {
          failure = 'Another tab changed the saved schedule change. Reopen this form to recover that request.';
          transaction.abort(); return;
        }

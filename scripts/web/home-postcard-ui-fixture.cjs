...
const [container,project,cli,evidence,portText='18084',surface='web']=process.argv.slice(2);
assert(['web','native'].includes(surface));
...
let fault=null,held=null,holdNext=null,server,nativeApp,schema=false,initialized=false,closing=false;
const events=[];
f.useDatabaseClient({supabaseUrl:client.supabaseUrl,async rpc(name,args){
 ...
 if(fault?.name===name&&fault.kind==='before'){if(!fault.persistent)fault=null;return {data:null,error:{code:'SYNTHETIC'}};}
 ...
 if(fault?.name===name&&!result.error){const kind=fault.kind;if(!fault.persistent)fault=null;if(kind==='malformed')return {data:{ok:true},error:null};throw Error('Synthetic lost committed result');}
 return result;
...
f.app.post('/fixture/fault',(req,res)=>{assert([...functions,'home_record_context'].includes(req.body.name));assert(['before','lost','malformed','clear'].includes(req.body.kind));fault=req.body.kind==='clear'?null:req.body;res.json({ok:true});});
...
async function cleanup(){
 ...
 nativeApp?.locals.releaseAdmission({discard:true});
...
async function main(){try{
 ...
 const app=surface==='native'?(nativeApp=require('../ios/home-postcard-native-boundary.cjs')(f,actor,events)):f.app;
 server=app.listen(port,'127.0.0.1');await new Promise((resolve,reject)=>{server.once('listening',resolve);server.once('error',reject);});console.log('READY: owned local postal UI fixture at '+port);

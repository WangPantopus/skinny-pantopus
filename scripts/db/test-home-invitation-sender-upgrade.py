#!/usr/bin/env python3
"""Rehearse the additive invitation sender migration with complete row-value preservation.
...
migration = (root / 'supabase/migrations/20260912040000_home_invitation_sender_recovery.sql').read_text()
query = r"""
...
DO $$ DECLARE t record; BEGIN
  ASSERT to_regclass('public."HomeInvitationSenderCommand"') IS NULL;
...
DO $$ DECLARE t record; actual_count bigint; actual_hash text; BEGIN
  ...
  END LOOP;
  ASSERT (SELECT count(*)=0 FROM public."HomeInvitationSenderCommand");
END $$;
...
WHERE n.nspname='public' AND p.proname IN('prepare_home_invitation_sender','list_home_invitation_sender','get_home_invitation_sender','resolve_home_invitation_sender','claim_home_invitation_sender_delivery','record_home_invitation_sender_delivery','act_on_home_invitation');

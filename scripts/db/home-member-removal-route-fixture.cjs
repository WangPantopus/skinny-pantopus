// Dedicated removal candidate route adapter, derived from accepted residency harness.
// Production routers/services with candidate SDK/SQL; synthetic auth
// and notification transport only; never accepts a hosted database or provider.
module.exports = function(container, { summary = false, place = false, dashboard = false, invitations = false, postcard = false, tasks = false, database } = {}) {
  ...
  assert.match(database || '', /^home_member_removal_r03_[a-f0-9]{12}$/);
  ...
  const transport = { createNotification: async value => {
    ...
    if (notificationFailure) { notificationFailure = false; throw new Error('Synthetic unavailable notification transport'); }
    return values.map((value,index)=>({id:id(950+index),user_id:value.userId}));
  } };
  ...
  Module._load = function(request, parent, isMain) {
    ...
    if (['/routes/homeIam.js','/routes/homeMemberRemovals.js'].some(suffix=>parent?.filename.endsWith(suffix)) && request === '../middleware/verifyToken') return (req, _res, next) => { const userId=req.headers['x-fixture-actor'];assert(users.includes(userId));req.user={id:userId};req.session={id:req.headers['x-fixture-session']};next(); };
  ...
  app.use('/api/homes', require(path.join(root, 'backend/routes/homeIam')));app.use('/api/homes', router);
  function setup() {
  ...
  function cleanup() {
    sql(`BEGIN;
      DELETE FROM public."HomeMemberRemovalCommand" WHERE actor_user_id IN (${users.map(q)});
      DELETE FROM public."HomeInvitationSenderCommand" WHERE actor_user_id IN (${users.map(q)});
      DELETE FROM public."HomeInvitationDecisionCommand" WHERE actor_user_id IN (${users.map(q)});
      DELETE FROM public."HomeScopedGrant" WHERE home_id=${q(home)};
      DELETE FROM public."ResidencyLetter" WHERE home_id=${q(home)};
  ...
  return { app, actor, home, claims, users, id, q, sql, scope, setup, cleanup, notifications, invitationEmailAttempts,
    ...
    useDatabaseClient(client) { const url = new URL(client.supabaseUrl); assert.equal(url.hostname, '127.0.0.1'); assert.equal(url.protocol, 'http:'); assert.equal(url.port, '18085'); databaseClient = client; },

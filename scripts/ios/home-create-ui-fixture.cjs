...
const count = () => Number(sql('SELECT ' + tables.map(t => `(SELECT count(*) FROM public."${t}" WHERE id::text LIKE 'ddc24100-%')`).join('+') + `+(SELECT count(*) FROM auth.users WHERE id=${q(actor)});`));
...
sql('BEGIN;'+migration+"NOTIFY pgrst, 'reload schema'; COMMIT;");
...
let heldProvider = null;
const db = { async rpc(name, args) {
  assert(['begin_home_create_command','get_home_create_command','cancel_home_create_command','finish_home_create_attempt','commit_home_create_command','home_record_context','home_delete_eligibility'].includes(name));
  ...
  assert(['Home', 'HomeOccupancy', 'HomeOwner', 'HomeOwnershipClaim', 'HomeVerificationEvidence', 'HomeRolePermission', 'HomePermissionOverride', 'AddressVerificationAttempt', ...tables].includes(table));
...
const databaseState = () => JSON.parse(sql(`SELECT json_build_object(
  ...
  'homes', (SELECT coalesce(json_agg(json_build_object('id',h.id,'unit',h.address2,'owner_id',h.owner_id,
    ...
    FROM public."Home" h WHERE created_by_user_id=${q(actor)}));`));
const state = () => ({ mode, events, held: !!held, held_provider: !!heldProvider, row_count: count(), database: databaseState() });
app.use((req, res, next) => {
  ...
  if (p === '/fixture/reset' || p === '/fixture/mode') {
    ...
    assert(['current', 'provider_outage', 'missing_unit', 'search_error', 'search_empty', 'lookup_error', 'malformed_validation', 'hold_provider', 'lost_commit_reply', 'finish_unavailable'].includes(nextMode));
    mode = nextMode; log({ event: 'mode', mode }); return res.json(state());
  }
  if (p === '/fixture/hold') { assert(['/validate', '/check-address', '/autocomplete', '/api/homes'].includes(req.body.suffix)); holdSuffix = req.body.suffix; return res.json(state()); }
...
async function stop() {
  ...
  if (initialized) sql(`BEGIN; DELETE FROM public."Home" WHERE created_by_user_id=${q(actor)}; DELETE FROM public."HomeCreateCommand" WHERE actor_user_id=${q(actor)};` + tables.map(t => `DELETE FROM public."${t}" WHERE id::text LIKE 'ddc24100-%';`).join('\n') + `DELETE FROM auth.users WHERE id=${q(actor)};COMMIT;`);
  ...
  sql('BEGIN;'+signatures.map(sig => 'DROP FUNCTION public.'+sig+';').join('\n')+'DROP TABLE public."HomeCreateCommand";'+"NOTIFY pgrst, 'reload schema'; COMMIT;");

...
let initialized=false,server,mode='current',events=[],periods;
const state=()=>({home_id:home,mode,periods,events});
function reset(){ if(initialized)f.cleanup(); periods=f.setup(); initialized=true;mode='current';events=[]; }
const access=()=>{
